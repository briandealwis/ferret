///* Copied from org.eclipse.jdt.internal.core.search.processing.JobManager */
/*******************************************************************************
...
public class JobManager implements Runnable {
	...
	/* place to send debug log messages */
	public static PrintStream log = System.out;
	public static boolean VERBOSE = FerretPlugin.hasDebugOption("debug/showJobDetails");
	...
	public JobManager(String processName) {
	...
	public void disable() {
		this.enableCount--;
		if (VERBOSE)
			verbose("DISABLING background indexing"); //$NON-NLS-1$
	}
	/**
	...
	public void discardJobs(String jobFamily) {
		...
		if (VERBOSE)
			verbose("DISCARD   background job family - " + jobFamily); //$NON-NLS-1$
	...
	public synchronized void enable() {
		this.enableCount++;
		if (VERBOSE)
			verbose("ENABLING  background indexing"); //$NON-NLS-1$
	...
	public boolean performConcurrentJob(IJob searchJob, int waitingPolicy, IProgressMonitor progress) {
		if (VERBOSE)
			verbose("STARTING  concurrent job - " + searchJob); //$NON-NLS-1$
			...
			if (awaitingJobsCount() > 0) {
				switch (waitingPolicy) {
					...
					case IJob.ForceImmediate :
						...
						if (VERBOSE)
							verbose("FINISHED  concurrent job - " + searchJob); //$NON-NLS-1$
						return status;
	
					case IJob.CancelIfNotReady :
						if (VERBOSE)
							verbose("-> NOT READY - cancelling - " + searchJob); //$NON-NLS-1$
						if (VERBOSE)
							verbose("CANCELED concurrent job - " + searchJob); //$NON-NLS-1$
							...
							try {
								...
								while ((awaitingJobsCount = awaitingJobsCount()) > 0) {
									...
									if (currentJob != null && currentJob != previousJob) {
										...
										if (subProgress != null) {
											subProgress.subTask(Integer.toString(awaitingJobsCount) + " jobs remaining");
	...
	public synchronized void request(IJob job) {
		...
		if (VERBOSE) {
			verbose("REQUEST   background job - " + job); //$NON-NLS-1$
			verbose("AWAITING JOBS count: " + awaitingJobsCount()); //$NON-NLS-1$
		}
		...
		try {
			class ProgressJob extends Job {
				...
				protected IStatus run(IProgressMonitor monitor) {
					...
					while (!monitor.isCanceled() && (awaitingJobsCount = awaitingJobsCount()) > 0) {
						monitor.subTask(Integer.toString(awaitingJobsCount) + " more jobs"); 
						try {
							Thread.sleep(500);
						} catch (InterruptedException e) {
							// ignore
			...
			while (this.processingThread != null) {
				try {
					...
					if (VERBOSE) {
						verbose(awaitingJobsCount() + " awaiting jobs"); //$NON-NLS-1$
						verbose("STARTING background job - " + job); //$NON-NLS-1$
					}
					try {
						this.executing = true;
						if (this.progressJob == null) {
							this.progressJob = new ProgressJob("Job in Progress"); 
							this.progressJob.setPriority(Job.LONG);
							this.progressJob.setSystem(true);
							this.progressJob.schedule();
						}
						/*boolean status = */job.execute(new NullProgressMonitor());
						//if (status == FAILED) request(job);
					} finally {
						this.executing = false;
						if (VERBOSE)
							verbose("FINISHED background job - " + job); //$NON-NLS-1$
						moveToNextJob();
	...
	public void discardJob(IJob job) {
		if(job == null) { return; }
		// flush and compact awaiting jobs
		try {
			int loc = -1;
			...
			synchronized(this) {
				for (int i = this.jobStart; i <= this.jobEnd; i++) {
					IJob currentJob = this.awaitingJobs[i];
					if (currentJob != null) { // sanity check
						this.awaitingJobs[i] = null;
						if (job == currentJob) { // copy down, compacting
							this.awaitingJobs[++loc] = currentJob;
						} else {
							if (VERBOSE) {
								verbose("-> discarding background job  - " + currentJob); //$NON-NLS-1$
		...
		if (VERBOSE) {
			verbose("DISCARD   DONE with background job - " + job); //$NON-NLS-1$
		}
	}	
}
