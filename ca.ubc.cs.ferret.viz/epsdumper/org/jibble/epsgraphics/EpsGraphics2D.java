...
$Author: bsd $
$Id: EpsGraphics2D.java,v 1.1 2007/09/20 19:11:12 bsd Exp $
...
public class EpsGraphics2D extends java.awt.Graphics2D {
        ...
        if (s != null) {
            ...
            while (!it.isDone()) {
                ...
                if (type == PathIterator.SEG_CLOSE) {
                    ...
                    append(x1 + " " + y1 + " " + x1 + " " + y1 + " " + x2 + " " + y2 + " curveto");
                }
    ...
    public void setStroke(Stroke s) {
        if (s instanceof BasicStroke) {
            ...
            append(_stroke.getMiterLimit() + " setmiterlimit");
    ...
    public Rectangle getClipBounds() {
        ...
        Rectangle rect = _clip.getBounds();
        return rect;
    }
    ...
    public boolean drawImage(Image img, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2, Color bgcolor, ImageObserver observer) {
        ...
        matrix.scale(1, -1);
        matrix.getMatrix(m);
        append(width + " " + height + " 8 [" + m[0] + " " + m[1] + " " + m[2] + " " +  m[3] + " " + m[4] + " " + m[5] + "]");
        // Fill the background to update the bounding box.
        Color oldColor = getColor();
        setColor(getBackground());
        fillRect(dx1, dy1, destWidth, destHeight);
        setColor(oldColor);
        append("{currentfile 3 100 mul string readhexstring pop} bind");
